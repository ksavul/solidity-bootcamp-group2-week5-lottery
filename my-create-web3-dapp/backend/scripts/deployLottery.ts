...
async function main() {
    ...
    const contract = await contractFactory.deploy("Lottery", "SYM", TOKEN_RATIO, ethers.parseUnits(BET_PRICE, 18),ethers.parseUnits(BET_FEE, 18));
    ...
    const token = await tokenFactory.deploy("LotteryToken", "TKN");
    await token.waitForDeployment();
    const tokenAddress = await token.getAddress();
    console.log(`Token lottery contract deployed at ${tokenAddress}`);
}
